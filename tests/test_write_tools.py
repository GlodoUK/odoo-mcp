...
from odoo_mcp.error_handling import ValidationError
from odoo_mcp.odoo_connection import OdooConnectionError
...
class TestWriteTools:
    ...
    @pytest.fixture
    def tool_handler(self, mock_app, mock_connection, mock_config):
        """Create OdooToolHandler instance."""
        return OdooToolHandler(mock_app, mock_connection, mock_config)
    ...
    def test_tools_registered(self, mock_app, mock_connection, mock_config):
        """Test that write tools are registered when not readonly."""
        ...
        mock_config.readonly = False

        register_tools(mock_app, mock_connection, mock_config)
    ...
    def test_write_tools_not_registered_when_readonly(self, mock_app, mock_connection, mock_config):
        """Test that write tools are NOT registered when readonly=True."""
        # Track functions that were decorated
        decorated_functions = []
        ...
        assert "create_record" not in decorated_functions
        assert "update_record" not in decorated_functions
        assert "delete_record" not in decorated_functions
...
class TestWriteToolsIntegration:
    ...
    def real_connection(self, real_config):
        ...
        if real_config.api_version == "json2":
            ...
            from odoo_mcp.odoo_connection import OdooConnection
    ...
    @pytest.fixture
    def real_tool_handler(self, real_app, real_connection, real_config):
        """Create real tool handler."""
        return register_tools(real_app, real_connection, real_config)
    ...
    async def test_create_update_delete_cycle(self, real_config, real_tool_handler):
        ...
        if real_config.readonly:
            pytest.skip("Write integration test requires ODOO_READONLY=false")
        ...
        try:
            ...
            # Verify deletion
            from odoo_mcp.tools import ValidationError
