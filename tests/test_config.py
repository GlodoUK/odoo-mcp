...
from odoo_mcp.config import OdooConfig, get_config, load_config, reset_config, set_config
...
class TestOdooConfig:
    ...
    def test_valid_config_with_xmlrpc_api_key(self):
        """Test creating a valid configuration with username:api_key format for XML-RPC."""
        config = OdooConfig(
            url="https://odoo.example.com",
            api_key="admin:myapikey123",
            database="test_db",
        )
        assert config.url == "https://odoo.example.com"
        assert config.api_key == "admin:myapikey123"
    ...
    def test_missing_authentication_raises_error(self):
        ...
        with pytest.raises(ValueError, match="ODOO_API_KEY is required"):
...
class TestLoadConfig:
    ...
    def test_load_config_from_env_file(self, monkeypatch):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".env", delete=False) as f:
            ...
            f.write("ODOO_API_KEY=fileuser:fileapikey\n")
        ...
        try:
            ...
            assert config.api_key == "fileuser:fileapikey"
...
class TestReadonlyMode:
    ...
    def test_readonly_default_true(self):
        """Test readonly is enabled by default."""
        config = OdooConfig(url="http://localhost:8069", api_key="test")
        assert config.readonly is True

    def test_readonly_can_be_disabled(self):
        """Test readonly can be set to False."""
        config = OdooConfig(url="http://localhost:8069", api_key="test", readonly=False)
        assert config.readonly is False

    def test_readonly_from_env_default(self, monkeypatch):
        """Test readonly defaults to True when ODOO_READONLY is not set."""
        monkeypatch.setenv("ODOO_URL", "http://localhost:8069")
        monkeypatch.setenv("ODOO_API_KEY", "test_api_key")
        monkeypatch.delenv("ODOO_READONLY", raising=False)

        config = load_config()
        assert config.readonly is True

    def test_readonly_from_env_false(self, monkeypatch):
        """Test readonly can be disabled via ODOO_READONLY=false."""
        monkeypatch.setenv("ODOO_URL", "http://localhost:8069")
        monkeypatch.setenv("ODOO_API_KEY", "test_api_key")
        monkeypatch.setenv("ODOO_READONLY", "false")

        config = load_config()
        assert config.readonly is False

    def test_readonly_from_env_true(self, monkeypatch):
        """Test readonly stays True when ODOO_READONLY=true."""
        monkeypatch.setenv("ODOO_URL", "http://localhost:8069")
        monkeypatch.setenv("ODOO_API_KEY", "test_api_key")
