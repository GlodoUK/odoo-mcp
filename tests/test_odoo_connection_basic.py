...
from odoo_mcp.odoo_connection import OdooConnection, OdooConnectionError, create_connection
...
class TestOdooConnectionInit:
    ...
    def test_build_endpoint_url(self, test_config):
        ...
        # Use the hardcoded endpoint paths from the connection
        from urllib.parse import urlparse

        parsed = urlparse(test_config.url)

        db_url = conn._build_endpoint_url(conn.DB_ENDPOINT)
        expected_url = f"{parsed.scheme}://{parsed.netloc}{conn.DB_ENDPOINT}"
        assert db_url == expected_url

        common_url = conn._build_endpoint_url(conn.COMMON_ENDPOINT)
        expected_common_url = f"{parsed.scheme}://{parsed.netloc}{conn.COMMON_ENDPOINT}"
        assert common_url == expected_common_url

        object_url = conn._build_endpoint_url(conn.OBJECT_ENDPOINT)
        expected_object_url = f"{parsed.scheme}://{parsed.netloc}{conn.OBJECT_ENDPOINT}"
