...
class TestXMLRPCOperations:
    ...
    def test_api_key_parsed_portion_used_for_execute_kw(self, config):
        """Test that the parsed api_key portion (not full username:api_key) is used for execute_kw."""
        config = OdooConfig(
            url=os.getenv("ODOO_URL", "http://localhost:8069"),
            api_key="admin:myapikey123",
            database=os.getenv("ODOO_DB", "db"),
        )
        conn = OdooConnection(config)
        conn._connected = True
        conn._authenticated = True
        conn._uid = 2
        conn._database = os.getenv("ODOO_DB", "db")
        conn._api_key = "myapikey123"  # parsed api_key portion set after authenticate()
        ...
        # Verify only the parsed api_key is used, not the full "admin:myapikey123"
        mock_proxy.execute_kw.assert_called_once_with(
            os.getenv("ODOO_DB", "db"), 2, "myapikey123", "res.partner", "search", [[]], {}
        )
