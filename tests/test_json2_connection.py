...
from odoo_mcp.odoo_json2_connection import OdooConnectionError, OdooJSON2Connection
...
class TestOdooJSON2Lifecycle:
    ...
    def test_authenticate_no_api_key_http_bearer_mode(self, json2_config):
        """Without an api_key, authenticate() succeeds (HTTP bearer mode)."""
        conn = OdooJSON2Connection(json2_config)
        conn._connected = True
        ...
        # Should not raise — defers auth to request time
        conn.authenticate()
...
class TestOdooJSON2ORM:
    ...
    def test_read_without_fields(self, connected_json2):
        ...
        _result = conn.read("res.partner", [1])
    ...
    def test_fields_get_with_attributes_not_cached(self, connected_json2):
        ...
        _result = conn.fields_get("res.partner", attributes=["string"])
