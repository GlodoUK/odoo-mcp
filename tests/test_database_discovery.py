...
class TestDatabaseDiscovery:
    ...
    def test_authenticate_success(self, connection):
        """Test authentication with username:api_key format."""
        connection._connected = True
        mock_common = Mock()
        ...
        # Verify common.authenticate was called with parsed username and api_key
        mock_common.authenticate.assert_called_once_with("testdb", "test", "api_key", {})

    def test_authenticate_with_xmlrpc_api_key(self):
        """Test authentication with username:api_key format parses correctly."""
        config = OdooConfig(
            url=os.getenv("ODOO_URL", "http://localhost:8069"),
        ...
        assert connection.is_authenticated
        assert connection._api_key == "myapikey123"
        mock_common.authenticate.assert_called_once_with("testdb", "admin", "myapikey123", {})

    def test_authenticate_returns_false_raises_error(self):
        """Test authentication raises error when Odoo returns False."""
        config = OdooConfig(
            url=os.getenv("ODOO_URL", "http://localhost:8069"),
            api_key="admin:wrongkey",
            database="testdb",
        )
        connection = OdooConnection(config)
        connection._connected = True
    ...
    def test_authenticate_fault_raises_error(self):
        """Test authentication raises error on XML-RPC fault."""
        config = OdooConfig(
            url=os.getenv("ODOO_URL", "http://localhost:8069"),
