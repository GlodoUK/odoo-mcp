...
class OdooMCPServer:
        ...
        if not self.connection:
            try:
                ...
                with perf_logger.track_operation("connection_setup"):
                    ...
                    # Connect (and authenticate for stdio; HTTP uses per-request bearer)
                    self.connection.connect()
                    if self.config.transport == "stdio":
                        self.connection.authenticate()
                    else:
                        # HTTP transport: set database from config, defer auth to requests
                        if self.config.api_version == "json2":
                            self.connection.authenticate()  # no-op sets database
                        elif self.config.database:
                            self.connection._database = self.config.database
    ...
    def _register_resources(self):
        ...
        self.resource_handler = register_resources(self.app, self.connection, self.config)
        logger.info("Registered MCP resources")

    def _register_tools(self):
        """Register tool handlers after connection is established."""
        self.tool_handler = register_tools(self.app, self.connection, self.config)
    ...
    async def run_http(self, host: str = "localhost", port: int = 8000):
        ...
        The incoming HTTP Authorization: Bearer <token> is forwarded to
        Odoo as-is — no token validation in the MCP server itself.
        ...
        try:
            ...
            # Get the Starlette app and wrap with BearerExtractMiddleware
            starlette_app = self.app.streamable_http_app()
            wrapped = BearerExtractMiddleware(starlette_app)
