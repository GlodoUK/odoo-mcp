...
class OdooResourceHandler:
    ...
    async def _handle_record_retrieval(self, model: str, record_id: str) -> str:
        ...
        try:
            with perf_logger.track_operation("resource_get_record", model=model):
                ...
                # Search to verify record exists
                record_ids = self.connection.search(model, [("id", "=", record_id_int)])

                if not record_ids:
                    raise NotFoundError(
                        f"Record not found: {model} with ID {record_id_int} does not exist",
                        context=context,
                    )
                ...
                result = formatter.format_record(records[0], fields_metadata)

                logger.info(f"Record retrieved: {model}/{record_id_int}")
                return result
    ...
    async def _handle_search(
        ...
        logger.info(f"Searching {model} records with domain: {domain}")

        try:
            with perf_logger.track_operation("resource_search", model=model):
                # Parse parameters
...
def register_resources(
    ...
    """
    handler = OdooResourceHandler(app, connection, config)
