...
class OdooJSON2Connection:
    ...
    def _build_headers(self) -> Dict[str, str]:
        ...
        Uses the per-request Bearer token from ContextVar if set (HTTP transport),
        otherwise falls back to the configured API key (stdio transport).

        Returns:
            Dict with Authorization, Content-Type, and X-Odoo-Database headers
        """
        token = current_bearer_token.get() or self.config.api_key
        headers = {
    ...
    def _call(self, model: str, method: str, **kwargs: Any) -> Any:
        ...
        try:
            response = self._client.post(url, json=body, headers=self._build_headers())
    ...
    def authenticate(self, database: Optional[str] = None) -> None:
        ...
        if not self.config.api_key:
            # HTTP bearer mode: token comes per-request via ContextVar
            self._authenticated = True
            logger.info(
                f"HTTP bearer mode: database '{self._database}', auth deferred to request time"
            )
            return

        # stdio mode: validate API key now by fetching user context
        try:
