...
class OdooToolHandler:
    ...
    async def _handle_list_models_tool(self) -> Dict[str, Any]:
        ...
        try:
            with perf_logger.track_operation("tool_list_models"):
                # Query models from ir.model, excluding transient and less useful system models
                domain = [
                    "&",
                    ("transient", "=", False),
                    "|",
                ...
                logger.info(f"Listed {len(models_list)} models from database")
    ...
    async def _handle_list_resource_templates_tool(self) -> Dict[str, Any]:
        ...
        try:
            # Get a sample of models for the response
            try:
                model_records = self.connection.search_read(
                    "ir.model",
                    [("transient", "=", False)],
                ...
                model_names = [r["model"] for r in model_records]
            except Exception:
                model_names = []
...
def register_tools(
    ...
    """
    handler = OdooToolHandler(app, connection, config)
